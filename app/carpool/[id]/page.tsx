...
export default function CarpoolDetailPage() {
  ...
  const [postOwner, setPostOwner] = useState<{ name: string; email: string } | null>(null);
  ...
  const loadPost = async () => {
    try {
      const response = await fetch(`/api/carpool?id=${params.id}`);
      if (response.ok) {
        const data = await response.json();
        setPost(data.post);
        // Fetch post owner info
        if (data.post?.user_id) {
          const userResponse = await fetch(`/api/user/${data.post.user_id}`);
          if (userResponse.ok) {
            const userData = await userResponse.json();
            setPostOwner({ name: userData.name, email: userData.email });
          }
        }
      }
    } catch (err) {
  ...
  const loadMessages = async () => {
    try {
      const response = await fetch(`/api/chat?postId=${params.id}&postType=carpool`);
      if (response.ok) {
        const data = await response.json();
  ...
  const checkContactExchange = async () => {
    ...
    try {
      const response = await fetch(
        `/api/contact?other_user_id=${post.user_id}&post_id=${params.id}&post_type=carpool`
      );
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <div className="container">
      ...
      <div className="posts-container">
        <Link href="/carpool" style={{ textDecoration: 'none', color: '#4a90e2', marginBottom: '16px', display: 'block' }}>
          ← Back to Carpool
        </Link>
